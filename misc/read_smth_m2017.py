...
#pv = np.arange(nx*ny*nz).astype(np.float32).reshape((nx,ny,nz))
...
# smooth P-vel 
print()
print('#######  Smoothing P-vels   ####################################')
print()

z_sig = 5*(20/dz) # tested at dz=20m and was good, so assume scale by that)
y_sig = z_sig*(dz/dy)
x_sig = z_sig*(dz/dx)
...
for ix in range(nx):
    for iy in range(ny):
        pv[ix,iy,:] = gaussian_filter1d(pv[ix,iy,:], z_sig)
    if (ix % perc_10) == 0:
...
pv = pv.transpose(0,2,1)
print('pv.shape:',pv.shape)
perc_10 = int(nx/10 + 0.5)
for ix in range(nx):
    for iz in range(nz):
        pv[ix,iz,:] = gaussian_filter1d(pv[ix,iz,:], y_sig)
    if (ix % perc_10) == 0:
...
for iz in range(nz):
    for iy in range(ny):
        pv[iz,iy,:] = gaussian_filter1d(pv[iz,iy,:], x_sig)
    if (iz % perc_10) == 0:
...
del pv # more cleanup
#tran_flat_pv = pv_float.transpose(2,1,0).flatten() #NOTE: transposed above while smoothing
tran_flat_pv = pv_float.flatten()
del pv_float # more cleanup

# this will write to a structured grid VTK file
gridToVTK('./test_smth_groningen_z%sm' %(int(dz)), x_c, y_c, z_c, pointData = {"P-vel" : tran_flat_pv})
