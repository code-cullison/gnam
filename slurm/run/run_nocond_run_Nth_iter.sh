...
###########################################################
#
# Step-4.5a: Clip Combined Kernels (alpha and beta)
#
###########################################################
CJID="afterok:$CJID"; 

JNAME="clip_ab_kernels";
OLOG="4.5a_${JNAME}.job.%N.%j.out";  # STDOUT
ELOG="4.5a_${JNAME}.job.%N.%j.err";  # STDERR
PROG="../N_iter/4.5a_clip_ab_kernels.slurm";
...
echo "CJID=\$(./spec_sbatch.sh -w=$CJID -o=$OLOG -e=$ELOG $NPU -lp=$LOG_PATH -jn=$JNAME -x=$PROG -xa=$STEP_F)" >> $RLOG;
CJID=$(./spec_sbatch.sh -w=$CJID -o=$OLOG -e=$ELOG $NPU -lp=$LOG_PATH -jn=$JNAME -x=$PROG -xa=$STEP_F);
